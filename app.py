...
# 🎯 অটোমেটিক টেবিল ফিক্সার লজিক (মিসিং কলাম যোগ করবে)
def init_db():
    try:
        conn, db_type = get_db_connection()
        cursor = conn.cursor()
        ...
        if db_type == 'postgres':
            ...
            ''')
            # 🎯 পুরনো টেবিল থাকলে photo_url কলাম জোরপূর্বক যুক্ত করা
            try:
                cursor.execute("ALTER TABLE users ADD COLUMN photo_url TEXT DEFAULT '';")
            except Exception:
                conn.rollback() # কলাম আগে থেকে থাকলে সমস্যা নেই
...
# ==========================================
# ২. Google Auth Sync API (Fail-Safe)
# ==========================================
@app.route('/api/google-auth-sync', methods=['POST'])
def google_auth_sync():
    data = request.get_json() or {}
    ...
    try:
        ...
        if existing_user:
            ...
            user_role = role
            # ব্যাকওয়ার্ড কম্প্যাটিবিলিটির জন্য নিরাপদ ইনসার্ট
            try:
                cursor.execute(
                    f"INSERT INTO users (email, name, category, photo_url) VALUES ({ph}, {ph}, {ph}, {ph})", 
                    (email, name, user_role, photo_url)
                ...
                conn.rollback()
                cursor.execute(
                    f"INSERT INTO users (email, name, category) VALUES ({ph}, {ph}, {ph})", 
...
def get_profile_data():
    ...
    try:
        ...
        if user:
            u_name = user['name'] if isinstance(user, dict) else user[0]
            u_role = user['category'] if isinstance(user, dict) else user[1]
            u_photo = user.get('photo_url', '') if isinstance(user, dict) else (user[2] if len(user) > 2 else '')
            return jsonify({"success": True, "name": u_name, "role": u_role, "photo_url": u_photo})
        else:
            return jsonify({"success": False, "error": "User not found"})
    except Exception as e:
